<?php if($unknown_list == false){ die('شما مجوز دسترسی به این صفحه را ندارید.'); } ?>
<h3 class="sub-header">لیست نا معلوم ها</h3>
معوقات دیروز
...
if(isset($_POST['change_payer_id'])){
	$update_id = $_POST['id'] ;
	$payer_id = $_POST['payer_id'] ;
	$update_query = "UPDATE `transaction` SET `payerId`='$payer_id' WHERE id = '$update_id' ;"; 
	...
	$transactions_query = "SELECT 
								...
								,users.mobile
								 FROM transaction 
									...
									WHERE (users.cart_pri is NULL OR users.cart_sec is NULL OR users.cart_ter is NULL)";
	$from = $yesterday;
	$to = $today;
	...
	while($transaction_row=$transaction_result->fetch_assoc()){
								...
								,users.mobile
								 FROM transaction 
									...
									LEFT JOIN users
											...
											OR transaction.pan = users.cart_sec AND transaction.payerId = users.payerId2
											OR transaction.pan = users.cart_ter AND transaction.payerId = users.payerId3
									WHERE (users.cart_pri is NULL OR users.cart_sec is NULL OR users.cart_ter is NULL) AND transaction.pan='$transactions_row[pan]'";
		...
		echo "
		  ...
		  <tr>
					...
					<td>$count</td>
					<td dir='ltr'><a href='index.php?page=unknown_list&pan=$transactions_row[pan]' target='_blank'>$transactions_row[pan]</a></td>
					<td class='col-xs-2'>
					
						<form class='form-inline' method='post'>
						$transactions_row[payerId] &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<input type='hidden' name='id' value='$transactions_row[test]'>
							...
							<input type='submit' name='change_payer_id' value='برو' class='btn btn-warning'>
						</form>
					</td>
					<td>$type</td>
					<td>$date_fa</td>
