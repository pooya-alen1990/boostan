<?php if($users_list_tabriz == false){ die('شما مجوز دسترسی به این صفحه را ندارید.'); } ?>
<h3 class="sub-header">لیست سهامداران تبریز</h3>
...
$i = 1 ;
	...
	$count_users="SELECT COUNT(id) FROM users WHERE city='تبریز' ;";
...
########################################
	...
	$every_body_query="SELECT * FROM users WHERE 1=1 AND city='تبریز' ";
	if(isset($_GET['id'])){
	$id_passed = $_GET['id'];
	...
	echo "
		  <tr>
					...
					<td><a href='index.php?page=transactions_list_tabriz&id=$every_body_row[id]' target='_blank'>$every_body_row[first_name] $every_body_row[last_name]</a></td>
...
if(!isset($_GET['id'])){
		...
		if($_GET['pages']<=($page_count-1)){
		  ...
		  echo "<li><a href='index.php?page=users_list_tabriz&pages=1'>اولین</a></li>";	
		  echo "<li><a href='index.php?page=users_list_tabriz&pages=$prev'>قبلی &raquo;</a></li>";
		  for($i=1;$i<=$page_count;$i++){
			  if($i == $_GET['pages']){$activepage = 'active';}else{$activepage = ' ';}
			echo "<li class='$activepage'><a href='index.php?page=users_list_tabriz&pages=$i'>$i</a></li>";
		  }
		  echo "<li><a href='index.php?page=users_list_tabriz&pages=$next'>&laquo; بعدی</a></li>";
		  echo "<li><a href='index.php?page=users_list_tabriz&pages=".$page_count."'>آخرین</a></li>";
}
?>  
</ul>
</div>