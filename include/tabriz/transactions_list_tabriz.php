<?php if($transactions_list_tabriz == false){ die('شما مجوز دسترسی به این صفحه را ندارید.'); } ?>
<h3 class="sub-header">لیست خرید های تبریز</h3>
خرید های دیروز تبریز
	...
	$transactions_query = "SELECT 
								...
								,users.mobile
								 FROM transaction 
									...
									WHERE 1 = 1 AND receptors.city='تبریز' ";
	$from = $yesterday;
	$to = $today;
	...
	while($transaction_row=$transaction_result->fetch_assoc()){
		...
		echo "
		  ...
		  <tr>
					...
					if($test=='ok'){
						echo "<td style='color:green;'>$transactions_row[first_name] $transactions_row[last_name]</td>";}
					else{
						echo "<td><a href='index.php?page=transactions_list_tabriz&id=$transactions_row[id]' target='_blank'>$transactions_row[first_name] $transactions_row[last_name]</a></td>";}
					echo "
...
<hr style="margin:20px 0;">
    ...
    <option value="all">همه</option>
            ...
            $receptors_query = "SELECT * FROM receptors WHERE city='تبریز' ; ";
...
$(document).ready(function() {
	var keyword = $("#receptor_name_ajax").val();
			$.post('include/tabriz/ajax_list_tabriz.php',{ keyword:keyword },function(data){
