<?php if($leaders_list == false){ die('شما مجوز دسترسی به این صفحه را ندارید.'); } ?>
<h3 class="sub-header">لیست سرتیم ها</h3>
...
################ END FUNCTION ###############
	...
	$marketers_query="SELECT DISTINCT marketers.first_name,marketers.last_name,marketers.id,marketers.mobile FROM 
						marketers INNER JOIN receptors
						ON marketers.id = receptors.marketers_id ;";
	...
	while($marketers_row=$marketers_result->fetch_assoc()){
		...
		$receptor_query="SELECT * FROM receptors WHERE marketers_id='$marketers_id'; ";
		$receptor_result=$mysqli->query($receptor_query);
		$receptor_count = $receptor_result->num_rows;

		$subTable = '<table class="table table-bordered table-striped table-condensed">
							<tr>
								<th>نام پذیرنده</th>
		...
		$subTable .= '</table>';
		  ...
		  echo "
		  <tr>
					<td style='vertical-align:middle;'>$marketers_id</td>
					<td style='vertical-align:middle;'><a href='index.php?page=marketers_list&id=$marketers_row[id]' target='_blank'>$marketers_row[first_name] $marketers_row[last_name]</a></td>
					<td style='vertical-align:middle;'>$marketers_row[mobile]</td>
					<td style='vertical-align:middle;'>$receptor_count</td>
					<td style='vertical-align:middle;'>$subTable</td>
		  </tr>
		  ";	
...
<?php
	$marketers_option = '';
	$marketers_select_query="SELECT * FROM marketers ; ";
	$marketers_select_result=$mysqli->query($marketers_select_query);
	while($marketers_select_row = $marketers_select_result->fetch_assoc()){
		$marketers_option .= "<option value='$marketers_select_row[id]'>".$marketers_select_row['first_name'].' '.$marketers_select_row['last_name'].'</option>';
		}
		
	$receptors_option = '';
	$receptors_select_query="SELECT * FROM receptors ORDER BY receptor_name ASC ; ";
	$receptors_select_result=$mysqli->query($receptors_select_query);
	while($receptors_select_row = $receptors_select_result->fetch_assoc()){
		$receptors_option .= "<option value='$receptors_select_row[id]'>".$receptors_select_row['receptor_name'].'</option>';
		}
		
	##### ADD #####
	if(isset($_POST['add'])){
		$marketers_select_id = $_POST['marketers_select_id'];
		$receptors_select_id = $_POST['receptors_select_id'];
		
		$add_select_query = "UPDATE `receptors` SET `marketers_id`='$marketers_select_id' WHERE id='$receptors_select_id' ; ";
		$add_select_result = $mysqli->query($add_select_query);
	}
	###############
	##### REMOVE #####
	if(isset($_POST['remove'])){
		...
		$remove_select_query = "UPDATE `receptors` SET `marketers_id`='0' WHERE id='$receptors_select_id' ; ";
		$remove_select_result = $mysqli->query($remove_select_query);
	}
	###############
?>
<form method="post" role="form" class="form-inline">
	<select class="form-control" name="marketers_select_id">
    	<?php echo $marketers_option; ?>
    </select>

	<select class="form-control" name="receptors_select_id">
    	<?php echo $receptors_option; ?>
    </select>
    <input type="submit" name="add" value="اضافه" class="form-control btn btn-success">
    <input type="submit" name="remove" value="حذف" class="form-control btn btn-warning">
</form>
