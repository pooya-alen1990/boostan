...
$getCredit = new SoapClient('http://melipayamak.ir/post/Send.asmx?wsdl', array('encoding'=>'UTF-8'));
		...
		elseif($settings_flag==0){
			...
			$count_sms = mysqli_num_rows($iterative_result);
				...
				if($count_sms==0){
					...
					$webServiceSignature = "6FB31095-4E0A-4743-8A08-023C4016E939";  
					$webServiceNumber   = "5000260134"; 
					$Mobiles      = array ($pooya_to); 
					$isFlash = false; 
						mb_internal_encoding("utf-8");
						 $textMessage=$pooya_message; 
...
if(isset($_POST['submit_iterative'])){
	...
	$fetch_query="SELECT 
								...
								,users.mobile
								 FROM transaction 
									...
									WHERE transaction.id > '$FromId' ORDER BY transaction.settlementDate , transaction.settlementTime ; ";
	...
	$fetch_result=mysqli_query($connection,$fetch_query);
		...
		while($row = mysqli_fetch_assoc($fetch_result)){
									...
									if($gender=='مرد'){$smsgender='آقای';}elseif($gender=='زن'){$smsgender='خانم';}
										$message="$smsgender $last_name\nسرمايه گذاري دیروز شما با کارت $pan و با شناسه واریز $payerId از $sub_category $receptor_name $amount ريال مي باشد.\nwww.scards.ir";
